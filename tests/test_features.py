...
class TestFeatures(TestCase):
    ...
    def test_standard_scaler_get_std(self):
        ...
        expected = np.array([0.5, 0.5])
        scaler.fit(data)
        calculated_std = np.std(data, axis=0)
        assert (calculated_std == expected).all(), "scaler fit does not return expected standard deviation {}. Got {}".format(expected, calculated_std)
